...
export const createProducto = async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO productos (codigo, descripcion, categoria, precio_compra)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
...
export const updateProducto = async (req, res) => {
  try {
    ...
    const { rows, rowCount } = await pool.query(
      `UPDATE productos
       SET codigo = $1, descripcion = $2, categoria = $3, precio_compra = $4, estado = $5
       WHERE id_producto = $6
       RETURNING *`,
      [
        codigo,
...
export const forceDeleteProducto = async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM detalle_entrada WHERE id_producto = $1', [
      idNum
    ])

    // Eliminar detalles de salida asociados
    await client.query('DELETE FROM detalle_salida WHERE id_producto = $1', [
      idNum
    ])
