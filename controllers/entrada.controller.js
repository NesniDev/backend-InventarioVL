...
export const createEntrada = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      'INSERT INTO entradas (proveedor) VALUES ($1) RETURNING id_entrada',
      [proveedor]
    )
    const id_entrada = rows[0].id_entrada

    for (let item of productos) {
      await client.query(
        `INSERT INTO detalle_entrada (id_entrada, id_producto, cantidad, precio_unitario)
         VALUES ($1, $2, $3, $4)`,
        [id_entrada, item.id_producto, item.cantidad, item.precio_unitario]
      )
    }
